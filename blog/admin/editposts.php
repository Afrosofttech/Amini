...
window.location.href = 'editposts.php?id=$id';</script>";
    ...
    exit();
        ...
        $queryupdate = "UPDATE posts SET title = '$post_title' , tag = '$post_tag' , content='$post_content' , 	status = '$post_status' , featured = '$post_featured' ,  image = '$picture' , postdate = '$post_date' WHERE id= '$post_id' " ;
...
?>
	...
	<div class="input-group">
		<label for="post_feature">Featured?</label>
	<select name="featured" class="form-control">
			<?php if($_SESSION['role'] == 'user') { echo "<option value='' ></option>"; } else { ?> 
        <option value="<?php  echo $post_featured; ?>"><?php  echo  $post_featured;  ?></option>>
        <?php
if ($post_featured == 'featured') {
	echo "<option value='Unfeatured'>Unfeature</option>";
} else {
	echo "<option value='featured'>feature</option>";
}
?>
<?php
}
?>
    ...
    <a type="button" name="cancel" class="btn btn-primary" href="posts.php" >Cancel</a>
